...
/*
Class SmartPtr:
Vai servir para conseguirmos estabelecer comparações nos elementos da BST.
*/
...
public:
	...
	///construtor com apontador nulo e forma de comparação vazia
	SmartPtr() : pointee(NULL) { comparison = new Comparison() };
	///@param pointee apontador 
	///construtor com apontador igual ao apontador pretendido pelo utlizador e forma de comparação vazia
	SmartPtr(T* pointee) : pointee(pointee) { comparison = new Comparison(); };
	///@param c forma de comparação
	///construtor com apontador nulo e a forma de comparaçãopretendida pelo utilizador
	SmartPtr(Comparison * c) : pointee(NULL) { comparison = new Comparison(c) };
	///@param pointee apontador
	///@param c forma de comparação
	///construtor com apontador e a forma de comparação escolhidos pelo utilizador
	SmartPtr(T* pointee, Comparison* c) : pointee(pointee) { comparison = new Comparison(*c); };
	///@param p2 elemento constante da classe SmartPtr
	///construtor em que a forma de comparação vai ser a forma de comparação de p2
	...
	///@param c forma de comparação
	///modifica a forma de comparação para c
	void setComparison(Comparison* c){ delete comparison; comparison = new Comparison(*c); };
	///@throw quando o apontador é nulo
	///@return apontador
	T& operator*(){ if (pointee == NULL) throw NullDereference(); return *pointee; };
	///@return apontador
	T* operator->(){ return pointee; };
	...
	///destrutor onde vai eliminar a comparação
	~SmartPtr(){ delete comparison; };
	///@param p2 elemento constante da classe p2
	///@return o apontador que se encontra na diferença entre os dois apontadores
	...
	int operator+(const SmartPtr& p2) const { return this->pointee + p2.pointee; };
	///@param i inteiro
	///@return o apontador que está i vezes atrás de pointee
	SmartPtr operator-(int i) const { return SmartPtr(this->pointee - i); };
	///@param i inteiro
	///@return o apontador que está i vezes à frente de pointee
	SmartPtr operator+(int i) const { return SmartPtr(this->pointee + i); };
	SmartPtr operator++(int) { SmartPtr(*this) out;  pointee++; return out; };
	SmartPtr& operator++() { ++pointee; return *this; };
	///@param c2 elemento constante da calsse SmartPtr
	///@return um novo elemento com o apontador e a forma de comparação iguais a c2
	...
	///@return verifica se o apontador é nulo
	bool isNull() const { return pointee == NULL; };
	///@throw se o apontador não existe
